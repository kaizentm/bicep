...
export class PasteAsBicepCommand implements Command {
  ...
  public async execute(
    ...
    try {
      ...
      // asdfg what if selection is partly in a string and partly not?
      //asdfg test with range
      let rangeStart =
        documentUri === editor.document.uri
          ? editor.document.offsetAt(editor.selection.active)
          : editor.document.offsetAt(
              new Position(Number.MAX_SAFE_INTEGER, Number.MAX_SAFE_INTEGER) //asdfg?
            );
      let rangeEnd =
        documentUri === editor.document.uri
      ...
      if (rangeEnd < rangeStart) {
        [rangeStart, rangeEnd] = [rangeEnd, rangeStart]; //asdfg test
      }

      if (editor?.document.languageId !== bicepLanguageId) {
        getLogger().debug(`${logPrefix}: Editor not bicep`);
        return;
      }

      const result = await this.callDecompileForPaste(
        context,
        documentUri, //asdfg special chars?
        document.getText(),
        rangeStart,
        rangeEnd - rangeStart,
        clipboardText,
      ...
      this.logCompletion(logPrefix, clipboardText, finalPastedBicep);
    }
  }
          ...
          if (
            ...
            try {
              ...
              if (canPasteResult.errorMessage || !canPasteResult.bicep) {
                ...
                getLogger().debug(`${logPrefix}: ${msg}`); //asdfg does this go to same output?
              ...
              // } catch (err) {
              //   await this.rejectCompletionPromisesAsdfg(err);
              //   throw err;
              ...
              //asdfg?
              // getLogger().debug(
              //   `${logPrefix}): Pasted "${clipboardText}" -> "${finalPastedBicep}"` //asdfg does this go to output channel?
              // );
              this.logCompletion(logPrefix, clipboardText, finalPastedBicep);
            }
          }
  ...
  /**
   * For testing - creates a promise that will be resolved when the next paste as bicep operation completes
   */
  // public static createCompletionPromise(): Promise<void> {
  //   // eslint-disable-next-line no-debugger
  //   debugger; //asdfg
  //   const globalObjects = this.asdfg();
  //   //asdfg globalObjects.__createCompletionPromise = "createCompletionPromise";

  //   // eslint-disable-next-line @typescript-eslint/no-explicit-any
  //   (<any /*asdfg?*/>globalObjects).___hi = "hi";

  //   const deferred = new Deferred<void>();
  //   globalObjects.pasteAsBicepDeferreds ??= [];

  //   //asdfg?
  //   globalObjects.pasteAsBicepDeferreds.push(deferred);
  //   return deferred.promise;
  // }

  // private async rejectCompletionPromisesAsdfg(reason: unknown): Promise<void> {
  //   // // eslint-disable-next-line no-debugger
  //   // debugger; //asdfg
  //   // const globalObjects = PasteAsBicepCommand.asdfg();
  //   // const deferreds = globalObjects.pasteAsBicepDeferreds;
  //   // globalObjects.pasteAsBicepDeferreds = [];
  ...
  private logCompletion(
    ...
    getLogger().debug(
      `${prefix}: Result: ${format(clipboardText)} -> ${bicepText}` //asdfg does this go to output channel?
    );
